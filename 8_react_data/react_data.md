...
`const nextPreview = URL.createObjectURL(*value*);`는 만들때마다 웹브라우져에 메모리를 할당한다. 이런 것을 side effect라고 한다. 그런데, 다른 파일을 선택하거나, 해제했을 때 메모리를 지워줘야지. 그게 아니면 계속 쌓인다. 

이 모든 것을 `useEffect` 로 한다. 
...
이것을 `return`에 넣어주면 된다. 여기서의 함수는 나중에 dependency리스트가 바뀌어서, 새로 콜백을 실행할 때, 그 전에 return을 실행해서 `side effect`를 정리할 수 있게 해 주는 것. 
